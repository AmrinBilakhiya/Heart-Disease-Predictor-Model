...
with tab3:
    ...
    Accuracies = list(data.values())   # FIXED variable name

    df = pd.DataFrame(list(zip(Models, Accuracies)), columns=['Models','Accuracies'])  # FIXED

    fig = px.bar(df, y='Accuracies', x='Models')  # FIXED spelling
    st.plotly_chart(fig)